"""
Portfolio weight optimization via scipy.optimize.minimize (SLSQP), subject
to: weights sum to 1, and each asset weight lies within [min_weight, max_weight].
...
find_min_loss_portfolio (previously named find_min_var_portfolio -- renamed
to reduce confusion)
    VaR in this project is stored as a NEGATIVE value (more negative = worse
    loss). So mathematically "maximizing VaR" is equivalent to "minimizing
    the magnitude of the tail loss". The old name find_min_var_portfolio
    was misleading -- it read as "minimize VaR", which would actually mean
    driving VaR toward -infinity (the worst possible outcome). The function
    is renamed to find_min_loss_portfolio to match what it actually does.
